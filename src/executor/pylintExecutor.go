...
type PylintExecutor struct {
	ToolInfo   // 匿名字段
	ReportPath string
}

// 占位符替换
func (b *PylintExecutor) replaceCommand(tmpdir string, scan_files []string) error {
	// 替换占位符
...
// NewPylintExecutor 创建新的 PylintExecutor 实例，在初始化时完成命令替换
func NewPylintExecutor(toolinfo ToolInfo, tmpdir string, scan_files []string) (*PylintExecutor, error) {

	pylintExecutor := &PylintExecutor{
		ToolInfo: ToolInfo{
			Name:               toolinfo.Name,
	...
	if err := pylintExecutor.replaceCommand(tmpdir, scan_files); err != nil {
		return nil, err
	}
	return pylintExecutor, nil
}

// GetToolInfo 返回工具信息
func (p *PylintExecutor) GetToolInfo() ToolInfo {
	return p.ToolInfo
}

// Execute 执行命令（命令已在初始化时替换完成，不需要参数）
func (p *PylintExecutor) Execute() *ExecutionResult {
	var cmd *exec.Cmd
	...
	if err != nil {
		if exitErr, ok := err.(*exec.ExitError); ok {
			...
			// 根据工具类型判断是否算成功
			success = isExitCodeAcceptable("pylint", exitCode)
		} else {
			// 真正的执行错误（命令找不到、权限问题等）
			exitCode = -1
			success = false
...
func (p *PylintExecutor) GetReportPath() string {
	return p.ReportPath
}
